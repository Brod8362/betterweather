...
class NextPhaseCommand(fn: () => Unit) extends CommandExecutor {
  override def onCommand(sender: CommandSender, command: Command, label: String, args: Array[String]): Boolean = {
    sender match {
      case p: Player =>
        ...
        p.sendMessage(s"${ChatColor.GREEN}Time advanced to just before next weather phase (${(timeUntilNext-200) max 200} ticks)")
        fn()
        true
      case _ =>
